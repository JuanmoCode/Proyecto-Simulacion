...
print("\n" + "="*70)
print(f"SIMULACIÓN CON REYNOLDS = {Re}, V0 = {V0}")
print("="*70)
...
V0:              {V0}
Convergió:       {converged}
Iteraciones:     {len(error_history)}
Error final:     {final_residual:.8e}
Tiempo:          {elapsed:.4f} s
Max vx:          {np.max(vx_final):.4f}
Max vy:          {np.max(vy_final):.4f}
""")
...
plt.imshow(
    ...
    extent=[0, Nx, 0, Ny],
    aspect='auto',
    norm=LogNorm(vmin=1e-6, vmax=velocity.max())
)
